...
import { CheckCircle, Circle, ExternalLink, Sparkles, X, Loader2 } from "lucide-react";
...
export function TaskboardPage() {
  ...
  const [tasks, setTasks] = useState<Task[]>(
    TASK_DEFINITIONS.map(t => ({ ...t, completed: false, isAnimating: false }))
  );
  const [loadingCompletions, setLoadingCompletions] = useState(true);

  // Load already-completed tasks from Supabase on mount
  useEffect(() => {
    if (!user) { setLoadingCompletions(false); return; }
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data && data.length > 0) {
          const completedIds = new Set(data.map(r => r.task_id));
          setTasks(prev => prev.map(t => ({ ...t, completed: completedIds.has(t.id) })));
        }
  ...
  const handleSubmitTask = async (task: Task) => {
    ...
    // Add XP to profile (also updates leaderboard via real-time)
    await addXP(task.xp);
    ...
    <div className="w-full max-w-7xl mx-auto p-6">
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-green-300 text-sm flex items-center gap-2">
                  ...
                  <span className="font-semibold text-yellow-400">+{gainedXP} XP</span> gained — leaderboard updated!
                </p>
              </div>
              <button onClick={() => setShowNotification(false)} className="text-green-400 hover:text-green-300">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
        ...
        <div className="relative bg-gradient-to-br from-blue-900/40 via-blue-800/30 to-blue-900/40 backdrop-blur-sm border-2 overflow-hidden"
          ...
          <div className="relative p-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-cyan-400 text-sm">Complete tasks to earn XP — your progress saves automatically</p>
            </div>
            <div className="text-right">
              <div className="text-sm text-cyan-400 mb-1">XP Still Available</div>
      ...
      <div className="mb-6 bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 border border-slate-700/50">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-cyan-400 font-semibold">{completedTasks.length} / {tasks.length} Tasks Completed</span>
        </div>
        <div className="relative h-2 bg-slate-700 rounded-full overflow-hidden">
          <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-cyan-500 to-blue-500 transition-all duration-700"
        ...
        {(["todo", "completed"] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`flex-1 px-6 py-3 rounded-lg border-2 font-semibold transition-all ${
              activeTab === tab
                ? tab === "todo" ? 'bg-cyan-500/20 border-cyan-400 text-cyan-400' : 'bg-green-500/20 border-green-400 text-green-400'
      ...
      {loadingCompletions ? (
        ...
        <div className="space-y-4">
          {activeTab === "todo" && (
            <>
              {todoTasks.map(task => (
                <div key={task.id} className={`relative transition-all duration-500 ${task.isAnimating ? 'opacity-0 scale-95 -translate-y-4' : 'opacity-100 scale-100 translate-y-0'}`}>
                  <div className={`relative bg-slate-800/50 backdrop-blur-sm rounded-lg p-6 border transition-all duration-300 ${task.isAnimating ? 'border-green-500 bg-green-500/10' : 'border-slate-700/50 hover:border-cyan-400/50'}`}>
                    {task.isAnimating && <div className="absolute inset-0 bg-green-500/20 rounded-lg animate-pulse"></div>}
                    <div className="relative flex items-start gap-4">
                      <div className="flex-shrink-0 mt-1">
                        {task.isAnimating ? <CheckCircle className="w-6 h-6 text-green-400 animate-pulse" /> : <Circle className="w-6 h-6 text-slate-500" />}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-xs font-mono text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded border border-cyan-400/30">{task.unit}</span>
                              <span className="text-xs font-semibold text-yellow-400 flex items-center gap-1"><Sparkles className="w-3 h-3" />+{task.xp} XP</span>
                            </div>
                        ...
                        <div className="flex gap-2 mt-4">
                          ...
                          <button onClick={() => handleSubmitTask(task)} disabled={task.isAnimating || !user}
              ...
              {todoTasks.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-400">Amazing work — check back for more tasks.</p>
                </div>
              )}
            </>
          )}
          ...
          {activeTab === "completed" && (
            <>
              {completedTasks.map(task => (
                <div key={task.id} className="relative bg-slate-800/30 backdrop-blur-sm rounded-lg p-6 border border-green-500/30">
                  <div className="flex items-start gap-4">
                    <CheckCircle className="w-6 h-6 text-green-400 flex-shrink-0 mt-1" />
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-mono text-green-400 bg-green-400/10 px-2 py-0.5 rounded border border-green-400/30">{task.unit}</span>
                        <span className="text-xs font-semibold text-yellow-400 flex items-center gap-1"><Sparkles className="w-3 h-3" />+{task.xp} XP</span>
                        <span className="text-xs text-green-400 ml-auto">✓ Completed</span>
                      </div>
                      <h3 className="text-lg font-semibold text-white/80 mb-1 line-through">{task.title}</h3>
              ...
              {completedTasks.length === 0 && (
                <div className="text-center py-16">
                  <Circle className="w-20 h-20 text-slate-600 mx-auto mb-4" />
